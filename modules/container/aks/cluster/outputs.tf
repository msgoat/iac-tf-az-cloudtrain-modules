...
output aks_disk_encryption_set_id {
  description = "Unique identifier of the Azure Disk Encryption Set the AKS cluster uses to encrypt node volumes and attached persistent volumes; only valid if `aks_disk_encryption_enabled` is true"
  value = var.aks_disk_encryption_enabled ? azurerm_disk_encryption_set.cmk_disk_encryption[0].id : ""
}