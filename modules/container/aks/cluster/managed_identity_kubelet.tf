...
locals {
  kubelet_identity_name = "id-${local.aks_cluster_name}-kubelet"
}
...
resource "null_resource" "wait_for_role_assignments_to_kubelet" {
  triggers = {
    ...
    allow_kubelet_to_access_secret_encryption_key_id = azurerm_role_assignment.allow_kubelet_to_access_secret_encryption_key.id
  }
}
